...
tracked_words = ["app", "movie", "meme", "story", "trick"]
...
def update_chart():
    """Update the live chart with the latest word counts."""
    ax.clear()
    ...
    ax.set_title("Real-Time Keyword Counts")
...
def process_message(message: str) -> None:
    try:
        ...
        if isinstance(message_dict, dict):
            ...
            # Count occurrences of each tracked word
            for word in tracked_words:
                word_counts[word] += text.count(word)

            logger.info(f"Updated word counts: {dict(word_counts)}")
